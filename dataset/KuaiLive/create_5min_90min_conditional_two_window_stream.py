"""
创建 5min-90min 两窗口 conditional emission 数据流。

设计约定：
- 保留所有原始样本的 5min 发送
- 只有 `staytime >= 5min` 的样本才会进入二阶段 90min 窗口发送
- 每条 5min 样本：
  - `send_timestamp = enter_time + 300`
  - `send_window_id = 300`
  - `culm_wt = min(real_watch_time, 300)`
- 每条 90min 样本：
  - `send_timestamp = enter_time + 5400`
  - `send_window_id = 5400`
  - `culm_wt = min(real_watch_time, 5400)`
- `send_window_id` 统一使用“秒”作为单位

默认输出：
- `./processed_data_stream/window2_5min_90min_conditional_emit.csv`
"""
...
def create_conditional_two_window_stream(
    ...
    part_90min = df[df[stay_col] >= FIRST_WINDOW_SEC].copy()
    part_90min["send_timestamp"] = part_90min[time_col] + SECOND_WINDOW_SEC
    part_90min["send_window_id"] = SECOND_WINDOW_SEC
    part_90min["culm_wt"] = np.minimum(part_90min[play_col], SECOND_WINDOW_SEC).astype(np.int64)

    out_df = pd.concat([part_5min, part_90min], ignore_index=True)
...
def main() -> None:
    ...
    ap.add_argument(
        "--output_csv",
        type=str,
        default="./processed_data_stream/window2_5min_90min_conditional_emit.csv",
        help="输出 CSV",
    )
    ap.add_argument(
        "--time_col",
