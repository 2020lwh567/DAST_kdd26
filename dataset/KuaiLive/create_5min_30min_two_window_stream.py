"""
创建 5min-30min 两窗口 full-space emission 数据流。

设计约定：
- 保留所有原始样本
- 每条样本都会发送两次：
  - 5min 窗口：`send_timestamp = enter_time + 300`，`send_window_id = 300`
  - 30min 窗口：`send_timestamp = enter_time + 1800`，`send_window_id = 1800`
- `send_window_id` 统一使用“秒”作为单位
- 5min 窗口的 `culm_wt = min(real_watch_time, 300)`
- 30min 窗口的 `culm_wt = real_watch_time`

默认输出：
- `./processed_data_stream/window2_5min_30min_full_space_emit.csv`
"""
...
def create_two_window_stream(
    ...
    for window_sec in WINDOWS_SEC:
        ...
        if window_sec == 300:
            part["culm_wt"] = np.minimum(part[play_col], 300).astype(np.int64)
        else:
...
def main() -> None:
    ...
    ap.add_argument(
        "--output_csv",
        type=str,
        default="./processed_data_stream/window2_5min_30min_full_space_emit.csv",
        help="输出 CSV",
    )
    ap.add_argument(
        "--time_col",
