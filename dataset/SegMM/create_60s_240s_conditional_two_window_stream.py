"""创建 SegMM 60s-240s 两窗口 conditional emission 数据流。"""
...
def create_conditional_two_window_stream(
    ...
    part_240s = df[df[stay_col] >= FIRST_WINDOW_SEC].copy()
    part_240s["send_timestamp"] = part_240s[time_col] + SECOND_WINDOW_SEC
    part_240s["send_window_id"] = SECOND_WINDOW_SEC
    part_240s["culm_wt"] = np.minimum(part_240s[play_col], SECOND_WINDOW_SEC).astype(np.int64)

    out_df = pd.concat([part_60s, part_240s], ignore_index=True)
...
def main() -> None:
    ...
    ap.add_argument("--output_csv", type=str, default="./processed_data_stream/window2_60s_240s_conditional_emit.csv")
