"""创建 SegMM 60s-120s 两窗口 full-space emission 数据流。"""
...
def create_two_window_stream(df: pd.DataFrame, time_col: str, play_col: str = "play_time_truncate") -> pd.DataFrame:
    ...
    for window_sec in WINDOWS_SEC:
        ...
        if window_sec == 60:
            part["culm_wt"] = np.minimum(part[play_col], 60).astype(np.int64)
        else:
...
def main() -> None:
    ...
    ap.add_argument("--output_csv", type=str, default="./processed_data_stream/window2_60s_120s_full_space_emit.csv")
